...
// pointConfig helper to describe the structure of a cloud of points
type PointConfig struct {
	NthRoot fr.Element
}

// PointOption defines option for the shape of a set of points.
type PointOption func(opt *PointConfig) error

// WithNthRoot is a PointOption telling that the given set of points is actually
// a set of nth roots of the argument elmt. It is assumed that all the roots are
// defined in the field.
func WithNthRoot(elmt fr.Element) PointOption {
	return func(opt *PointConfig) error {
		opt.NthRoot.Set(&elmt)
		return nil
	}
}
